...
# ICBT
df_icbt_wide <- df_long %>%
  filter(treatment == "ICBT") %>%
  select(subject, time, LSAS) %>%
  pivot_wider(names_from = time, values_from = LSAS) %>%
  mutate(change = pre - post)

d_icbt <- with(df_icbt_wide, mean(change, na.rm = TRUE) / sd(change, na.rm = TRUE))
d_icbt

# SSRI
df_ssri_wide <- df_long %>%
  filter(treatment == "SSRI") %>%
  select(subject, time, LSAS) %>%
  pivot_wider(names_from = time, values_from = LSAS) %>%
  mutate(change = pre - post)

d_ssri <- with(df_ssri_wide, mean(change, na.rm = TRUE) / sd(change, na.rm = TRUE))
d_ssri
