...
## Three-way interaction statistical power ####
#### Power analysis for three-way interaction (time × treatment × sleep) ####
### simr
## SP
library(simr)
m <- model_list$SP_z
...
row_name <- "timepost:treatmentSSRI:SP_z"
...
m <- model_list$NRS_z
coef_table <- summary(m)$coefficients
row_name <- "timepost:treatmentSSRI:NRS_z"
est <- coef_table[row_name, "Estimate"]
se  <- coef_table[row_name, "Std. Error"]
MDE_95 <- 1.96 * se
MDE_80 <- 0.84 * se
...
m <- model_list$apnea_z
coef_table <- summary(m)$coefficients
row_name <- "timepost:treatmentSSRI:apnea_z"
est <- coef_table[row_name, "Estimate"]
se  <- coef_table[row_name, "Std. Error"]
MDE_95 <- 1.96 * se
MDE_80 <- 0.84 * se
MDE_apnea <- data.frame(
  predictor = "apnea_z",
  interaction = row_name,
  estimate = est,
  SE = se,
...
m <- model_list$sleepiness_z
coef_table <- summary(m)$coefficients
row_name <- "timepost:treatmentSSRI:sleepiness_z"
est <- coef_table[row_name, "Estimate"]
se  <- coef_table[row_name, "Std. Error"]
MDE_95 <- 1.96 * se
MDE_80 <- 0.84 * se
MDE_sleepiness <- data.frame(
  predictor = "sleepiness_z",
  interaction = row_name,
  estimate = est,
  SE = se,
  MDE_95 = MDE_95,
...
m <- model_list$KSQ_total_z
coef_table <- summary(m)$coefficients
row_name <- "timepost:treatmentSSRI:KSQ_total_z"
est <- coef_table[row_name, "Estimate"]
se  <- coef_table[row_name, "Std. Error"]
MDE_95 <- 1.96 * se
MDE_80 <- 0.84 * se
MDE_KSQ <- data.frame(
  predictor = "KSQ_total_z",
  interaction = row_name,
  estimate = est,
  SE = se,
  MDE_95 = MDE_95,
...
m <- model_list$KSS_pre_z
coef_table <- summary(m)$coefficients
row_name <- "timepost:treatmentSSRI:KSS_pre_z"
est <- coef_table[row_name, "Estimate"]
se  <- coef_table[row_name, "Std. Error"]
MDE_95 <- 1.96 * se
MDE_80 <- 0.84 * se
...
write.csv(model_fit_info, "model_fit_information_with_R2.csv", row.names = FALSE)
...
model_results <- tidy(fit_overall, effects = "fixed")
write.csv(model_results, "LMM_results_fixed_effects.csv", row.names = FALSE)
...
emm_overall_df <- as.data.frame(emm_overall) %>%
  mutate(result_type = "EMMeans")

emm_pairs_df <- as.data.frame(emm_overall_pairs) %>%
  mutate(result_type = "Pre-post contrasts")

emm_treatment_df <- as.data.frame(emm_treatment_by_time) %>%
  mutate(result_type = "Treatment contrasts")
...
write.csv(emm_all, "EMMeans_all_results.csv", row.names = FALSE)
...
print(d_icbt, digits = 3)
print(d_ssri, digits = 3)

## Between group effect size
df_change <- df_long %>%
  select(subject, treatment, time, LSAS) %>%
  pivot_wider(names_from = time, values_from = LSAS) %>%
...
#### Sensitivity analysis: LSAS change score ####
df_change <- df %>%
  mutate(
...
## Figure 2: trajectories + group means + 95% CI
...
## Figure 3: combined treatment figure
p_combined <- ggplot() +
  geom_line(
    data = df_long,
