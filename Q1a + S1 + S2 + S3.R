...
run_logistic <- function(var_list, data) {
  ...
  results_list <- lapply(var_list, function(v) {
    ...
    formula <- as.formula(
      paste("group_fac01 ~", v, "+ age + sex + sample")
    )
    ...
    beta <- coef_table[v, "Estimate"]
    pval <- coef_table[v, "Pr(>|z|)"]
    ...
    ## outcome prevalence
    outcome_prev <- mean(data$group_fac01 == 1, na.rm = TRUE)
    
    ## predictor prevalence
    predictor_prev <- mean(data[[v]] == 1, na.rm = TRUE)
    
    power_obj <- pwrss.z.logreg(
      odds.ratio = OR,
      p0 = outcome_prev,
      p1 = predictor_prev,
      n = n,
    ...
    data.frame(
      ...
      lower = exp(cis[v, 1]),
      upper = exp(cis[v, 2]),
      power = power_obj$power
    )
  })
  
  main_results <- do.call(rbind, results_list)
  
  main_results$Bonf_raw <- p.adjust(
    main_results$p,
    method = "bonferroni"
  )
  ...
  if (show_bonf) {
    p <- p +
      geom_text(
        ...
        aes(label = ifelse(Bonf_raw < 0.001 | Bonf_raw == 0,
                           "Bonf<0.001",
                           paste0("Bonf=", sprintf("%.3f", Bonf_raw)))),
        hjust = -0.1,
        size = 3.4,
        color = "black"
      ) +
...
ggsave(
  "Forestplot_sleep_combined_sample.png",
  p_sleep,
  width = 6,
  height = 4.5,
  device = "png"
...
#### ROC for each significant variables (age+sex+sample) ####
library(pROC)
...
for (i in seq_along(sig_sleep_vars)) {
  ...
  formula_i <- as.formula(
    paste("group_fac01 ~", paste(c(var, "age", "sex", "sample"), collapse = " + "))
  )
  
  model_i <- glm(formula_i, data = df, family = binomial)
  prob_i <- predict(model_i, type = "response")
  
  roc_i_raw <- roc(df$group_fac01, prob_i, ci = TRUE, quiet = TRUE)
...
label_map <- setNames(
  paste0(auc_df$variable_raw,
         " (AUC = ", sprintf("%.3f", auc_df$auc), ")"),
  auc_df$variable_raw
)
...
p_multi_roc_sleep_raw <- ggplot(roc_all_raw,
  ...
  coord_equal() +
  labs(
    title = "ROC curves for individual sleep variables (adjusted for age + sex + sample)",
...
ggsave(
  "ROC_sleep_individual_combined_sample.png",
...
#### Multivariable logistic regression (MAIN MODEL) ####
...
formula_multi <- as.formula(
  paste(
    "group_fac01 ~",
    paste(c(sig_sleep_vars, "age", "sex", "sample"),
          collapse = " + ")
  )
)
...
chi_squared <- model_multi$null.deviance - model_multi$deviance

p_value <- pchisq(
  chi_squared,
  df = model_multi$df.null - model_multi$df.residual,
  lower.tail = FALSE
)
...
## simr requires a mixed-effects model structure.
## Therefore, an artificial subject ID is created so that
## the model can be fitted using glmer().

df$ID <- 1:nrow(df)

#### Refit the model using glmer for simulation-based power
...
#### 1) Overall power for the multivariable sleep model

## Reduced model:
## Includes only adjustment covariates

power_full_model <- powerSim(
...
#### 2) Power curve across sample sizes
pc_full <- powerCurve(
...
## VIF > 5 may indicate problematic multicollinearity
...
events <- sum(df$group_fac01 == 1)

## Number of predictors in the multivariable model
n_predictors <- 7

## EPV calculation
EPV <- events / n_predictors

EPV

### Statistical power (pwrss)
library(pwrss)

n_total <- nrow(df)
base_prob <- mean(df$group_fac01 == 1)

or_assumed <- 1.5

power_result <- power.z.logistic(
  base.prob = base_prob,
  odds.ratio = or_assumed,
  n = n_total,
  power = NULL,
  alpha = 0.05
)
...
or_values <- seq(1.1, 3.0, by = 0.05)
power_values <- sapply(or_values, function(or) {
  result <- power.z.logistic(
    base.prob = base_prob,
    odds.ratio = or,
    n = n_total,
    power = NULL,
...
mde_index <- which(power_values >= target_power)[1]
mde_or <- or_values[mde_index]

print(data.frame(OR = or_values, Power = round(power_values, 3)))

cat("\n========================================\n")
cat("Minimum Detectable Effect Size (MDE)\n")
cat("========================================\n")
cat("Sample size (N):", n_total, "\n")
cat("Baseline probability:", base_prob, "\n")
cat("Target power: 80%\n")
cat("Alpha:", alpha, "\n")
cat("\n")
cat("Minimum detectable OR =", mde_or, "\n")

#### McFadden pseudo-R² 

pseudo_r2 <- pR2(model_multi)
...
final_table$CI_95 <- paste0(
  final_table$CI_lower, "–", final_table$CI_upper
)
...
roc_sleep <- roc(df$group_num, prob_sig_sleep, ci = TRUE)
...
# Brier Score
brier_sleep <- mean((prob_sig_sleep - df$group_num)^2)
brier_sleep


#### ROC visualization (multivariable model)
roc_df <- data.frame(
...
p_roc_sleep <- ggplot(roc_df, aes(x = FPR, y = TPR)) +
  ...
  coord_equal() +
  labs(
    title = "ROC curve (multivariable logistic model)",
    x = "False positive rate",
    y = "True positive rate"
  ) +
  annotate(
    ...
    label = paste0(
      ...
      "\n95% CI: ",
      sprintf("%.3f", auc_ci_sleep[1]), "–",
      sprintf("%.3f", auc_ci_sleep[3]),
      "\nEPV = ", round(EPV, 2),
      "\nMcFadden R² = ", round(as.numeric(mcfadden_r2), 3)
    ),
    size = 4.2,
    label.size = 0.3
  ) +
...
ggsave(
  "ROC_multivariable_logistic.png",
  p_roc_sleep,
  width = 5.5,
  height = 5,
  device = "png"
...
## 1. Data preparation
analysis_vars <- c("SP", "NRS", "sleepiness", "KSS_pre", "age", "sex", "sample")
...
## repeatedcv: 10 folds x 10 repeats = 100 resamples
n_resamples <- 10 * 10
...
ggsave(
  "Calibration_logistic_vs_SVM_cv.png",
  plot = p_cal,
  width = 6,
  height = 5,
  device = "png"
...
vars_pca <- df_ml[, 
                  analysis_vars[!analysis_vars %in% c("sex", "sample")],
                  drop = FALSE
]
...
ggsave(
  "SVM_predicted_probability_distribution.png",
  plot = p_svm_prob,
  width = 5.5,
  height = 5,
  device = "png"
