...
#### baseline isi predicts LSAS changes ####
df <- df %>%
  mutate(treatment_outcome = LSAS_pre - LSAS_post)
library(tidyverse)
df_long_isi <- df %>%
  pivot_longer(cols = c(LSAS_pre, LSAS_post), 
               names_to = "time", 
               values_to = "LSAS") %>%
  mutate(time = factor(time, levels = c("LSAS_pre", "LSAS_post"), labels = c("pre", "post"))) %>%
  group_by(subject) %>%
  mutate(treatment_outcome = LSAS[time == "pre"] - LSAS[time == "post"]) %>%
  ungroup()

library(lme4)
model <- lmer(treatment_outcome ~ isi1 + age + sex + (1 | subject), data = df_long_isi)
summary(model)
library(sjPlot)
tab_model(model, show.est = TRUE, show.ci = TRUE, digits = 3)

## sensitivity check
df <- df %>%
  mutate(treatment_outcome = LSAS_pre - LSAS_post)
model_1 <- lm(treatment_outcome ~ isi1 + LSAS_pre + age + sex, data = df)
summary(model_1)
library(car)
vif(model_1)
