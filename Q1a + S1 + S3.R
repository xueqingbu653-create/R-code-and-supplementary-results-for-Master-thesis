...
# Bonferroni-significant variables (multivariables)
...
null_deviance <- model_multi$null.deviance
residual_deviance <- model_multi$deviance

df_null <- model_multi$df.null
df_residual <- model_multi$df.residual
chi_squared <- null_deviance - residual_deviance

p_value <- pchisq(chi_squared, df = df_null - df_residual, lower.tail = FALSE)
print(p_value)
...
#### ROC for the multivariable model ####
