...
sleep_results <- sleep_results %>%
  group_by(effect) %>%
  mutate(
    p_fdr = ifelse(effect=="interaction",
                   p.adjust(p,method="BH"),
                   NA),
    p_bonf = ifelse(effect=="interaction",
...
## multiple comparison correction by effect type
sleep_results_s2 <- sleep_results_s2 %>%
  group_by(effect) %>%
  mutate(
  ...
  ungroup() %>%
  select(group, variable, effect, term, beta, se, CI_lower, CI_upper, p, p_fdr, p_bonf, p_fwe, n)

brain_results_s2 <- brain_results_s2 %>%
  group_by(effect) %>%
  mutate(
    p_fdr  = p.adjust(p, method = "BH"),
    p_bonf = p.adjust(p, method = "bonferroni"),
...
write_xlsx(
  ...
  path = "LMM_sample2_main_time_interaction.xlsx"
)
