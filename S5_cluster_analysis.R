...
# Group difference among three subtypes

anova_baseline <- aov(LSAS_pre ~ subtype, data = sad_dat)
anova_summary <- summary(anova_baseline)

write.csv(as.data.frame(anova_summary[[1]]), "ANOVA_baseline_results.csv", row.names = FALSE)
...
lm_baseline_summary <- summary(lm_baseline)
write.csv(as.data.frame(lm_baseline_summary$coefficients), "LM_baseline_results.csv", row.names = TRUE)
...
lmm_subtype_summary <- summary(lmm_subtype)
write.csv(as.data.frame(lmm_subtype_summary$coefficients), "LMM_subtype_summary_results.csv", row.names = TRUE)
...
lmm_subtype_treatment_summary <- summary(lmm_subtype_treatment)
write.csv(as.data.frame(lmm_subtype_treatment_summary$coefficients), "LMM_subtype_treatment_summary_results.csv", row.names = TRUE)
...
# run post hoc tests for ROIs with nominal p < .05
...
# Optional case-control exploratory analysis in sample 1
# This exploratory analysis tests whether subtype and case-control group
# are jointly associated with ROI variability in sample 1
